...
def check_mic_works():
    ...
    #print(temp_file)
    #print(temp_path)
    #filename_new = "ISFH" + "_" + str(camera_id) + "_" + str(datetime.now().strftime('%Y-%m-%d-%H-%M-%S')[:-3]) + ".csv"
    file_name_new = "Voice" + "_" + str(camera_id) + "_" + str(datetime.now().strftime('%Y-%m-%d-%H-%M-%S')) +".wav"
    path_name = "/home/pi/~/joy_detection_voice/output/" + file_name_new
    os.close(temp_file)

    #input("When you're ready, press enter and say 'Testing, 1 2 3'...")
    print('Recording...')
    aiy.audio.record_to_wave(path_name, RECORD_DURATION_SECONDS)
