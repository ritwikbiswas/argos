...
# fb = firebase.FirebaseApplication('https://joydetection.firebaseio.com', None)
    ...
    def run(self, num_frames, preview_alpha, image_format, image_folder):
        ...
        try:
            ...
            with PiCamera(sensor_mode=4, resolution=(1640, 1232)) as camera, PrivacyLed(leds):
                ...
                time_interval = 1
                camera_id = 4
                filename_new = "Vision" + "_" + str(camera_id) + "_" + str(datetime.now().strftime('%Y-%m-%d-%H-%M-%S')) + ".csv"
                ...
                f.write("time_stamp, elapsed_time, camera_id, face_count, avg_FEQ, person1, person2, person3, person4, person5, person6, person7, person8, person9\n")
                file_write_str = ""
                with CameraInference(face_detection.model()) as inference:
                    ...
                    for i, result in enumerate(inference.run()):
                        ...
                        # print(elapsed_time_total_str)
                        time_stamp = str(t0.tm_hour) + ":" + str(t0.tm_min) + ":" + str(t0.tm_sec) + " | " + str(t0.tm_mon) + "/" + str(t0.tm_mday) + "/" + str(t0.tm_year)
                        time_stamp_now = datetime.now().strftime('%Y-%m-%d-%H:%M:%S.%f')[:-3]
                        time_up = time_stamp = str(t0.tm_hour) + ":" + str(t0.tm_min) + ":" + str(t0.tm_sec)
